...
// 📌 Web App URLs
const WEB_APP_URL = process.env.KENO_URL || "https://stunning-croquembouche-c49862.netlify.app";
...
if (BOT_TOKEN) {
    ...
    console.warn("⚠️ BOT_TOKEN አልተዋቀረም! Bot አይሰራም።");
}
...
if (bot) {
    ...
    bot.onText(/\/deposit_(\d+)_(\d+(\.\d+)?)/, (msg, match) => {
        ...
        if (ADMIN_ID && senderId !== String(ADMIN_ID)) return;
...
io.on('connection', (socket) => {
    ...
    socket.on('verifyAndDeposit', (data) => {
        ...
        // ማሳሰቢያ፦ የSMS ማረጋገጫ በግንባር አድሚን በኩል ቢሆን የበለጠ ደህንነቱ የተጠበቀ ነው
        const adminMsg = `⚠️ የቴሌብር SMS ማረጋገጫ ጥያቄ!\n\n👤 ተጫዋች: ${user.name}\n🆔 ID: ${userId}\n💰 መጠን: ${amount} ETB\n\n📝 SMS:\n${smsText}\n\nለማጽደቅ ይጠቀሙ፦\n/deposit_${userId}_${amount}`;
        
        if (bot && ADMIN_ID) {
            bot.sendMessage(ADMIN_ID, adminMsg);
        }
        socket.emit('infoMsg', 'የላኩት SMS ለAdmin ተልኳል፤ ተረጋግጦ ይጨመራል።');
    });
